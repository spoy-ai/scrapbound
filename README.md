# 缝隙求生 · SCRAPBOUND 0.3.1

原创俯视角废土玩具 Roguelike。布偶「补丁」带着瓶盖锯、纽扣和发条，在旧玩具堆中求生。支持无尽模式、三条构筑路线、场中委托、敌人碰撞连锁、主动暴走及原创动态配乐。

**0.3.1 重点打磨锯击手感**：重新编排蓄力、切入和收招，第三挥带出更明亮的重锯刀光；命中时加入 35–75 毫秒的短暂停顿、定向镜头回弹、敌人形变和顺着锯击方向散开的残骸。六个原创短音效区分轻挥、重挥、铁皮、布偶与拆解，使用独立声道，保持锯声清楚。

两种模式的常规出怪间隔缩短 6%（未触及间隔下限时，频率约增加 6.4%）。敌人的生命、伤害、招式预警及同时特殊攻击上限沿用原有数值。
...
![0.3.1 重锯命中实机画面](captures/v04/saw_demo-heavy.png)
...
[GitHub 仓库](https://github.com/spoy-ai/scrapbound) · [v0.3.1 发布](https://github.com/spoy-ai/scrapbound/releases/tag/v0.3.1) · [原可玩版 v0.3.0](https://github.com/spoy-ai/scrapbound/releases/tag/v0.3.0)

使用 GitHub 的提交记录和版本标签管理游戏。`v0.3.0` 保留上一版可玩基线；后续修改先在开发分支完成并提交，验证后合入主分支。需要回退时，取回对应的历史提交或版本标签即可。

首次从 GitHub 克隆或下载工程时，启动器会自动导入资源；本机需先安装 Godot 4 标准版。仓库包含全部运行美术、动作和音乐，原始素材及实机演示保存在发布页。缓存、临时录像和平台签名下载地址不进入仓库。

## 开源与资源使用

游戏代码、制作工具、配置和相关文字文档采用 [MIT License](LICENSE)。美术、动画、原创音乐、音效、游戏截图及实机录像采用 [CC BY 4.0](assets/LICENSE.md)，允许使用、修改、再分发和商用，并按对应许可证保留许可说明或署名。此资源许可也涵盖本项目 GitHub Releases 已发布的对应媒体与原素材包。

Noto Sans SC 字体继续遵循原有的 [SIL Open Font License 1.1](assets/FONT-LICENSE.txt)。具体授权范围、署名示例及第三方材料说明见 [资源许可](assets/LICENSE.md)。
...
保留 0.2 的六种普通敌人、独立首领、各自死亡动画、动态纽扣、电池、绷带、地毯和织物界面。0.3 增加六张关键玩法图片，以及音乐盒、回收机两段真实生成的机械循环动作。0.3.1 沿用这些资源，调整挥锯动作节奏，并增加绘制刀光、受击形变及原创音效。
...
- `tools/check_saw.gd`：41 项锯击结算、单次停顿上限、重锯余震、输入缓冲、全战斗计时、特效容量、音频池与难度幅度检查全部通过，报告见 `captures/v04/saw-checks.json`。
- `assets/v04/audio/audio-analysis.json`：六个新增原创音效的信号检查；生成过程和声音说明同目录保留。
- `captures/v04/重锯打击感实机演示.mp4`：13 秒、60 fps、含实际混音的锯击录像，使用预设场景自动操作。
- `captures/v03/无尽与配乐实机演示.mp4`：13 秒、60 fps、含实际游戏混音的实机录像；使用预设中期构筑和自动操作，展示纽扣缝线、敌人冲撞与发条暴走。
- `captures/v03/checks.json`：本次更新后重新通过 158 项完整规则回归，包括无尽时限、首领周期、三路线、蓝图实效、事件、幂等奖励、连锁、输入及音乐时钟。
